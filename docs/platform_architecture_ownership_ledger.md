...
| Regime perspective | Immutable bull, bear, or sideways perspective | Corresponding regime agent | Workflow-output projector when the perspective has approved independent historical value | RAG-eligible when reasoning is durable; graph links to synthesis | Do not persist every intermediate perspective automatically without an eligibility policy |
| Unified strategy recommendation | Immutable strategy synthesis result | `StrategySynthesisAgent` | Workflow-output projector → `RecommendationPersistenceService` and/or typed agent recommendation record according to the approved record contract | Strong Qdrant candidate; graph links to signals, risk, events, portfolio, and regime perspectives | No parallel recommendation schema owned by reports or portfolio management |
