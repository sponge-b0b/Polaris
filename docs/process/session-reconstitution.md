...
The final fallback applies to **all workflow steps whose only unavailable capability is separate model identity/context**, including a `$spec-contract` build that ordinarily requires a genuinely fresh builder. If the active ChatGPT instance has already seen a prior contract hash, manifest, mapping, receipt, handoff, or related historical state, that prior exposure does not by itself block the owner-authorized fallback. The substitute must rebuild from the current allowed authoritative inputs, must not use the historical representation as an expected answer or construction input, and may compare historical identity only after the substitute build has produced its own terminal result. All deterministic schema, hashing, handoff, exact-HEAD, coherence, and downstream certification requirements remain in force.
...
#### Recover lost structured handoffs from canonical producers
...
* **`$spec-contract` same-instance fallback:** when the proven genuinely fresh model substrate is unavailable after the targeted capability check required above, ChatGPT is explicitly authorized to perform the current `$spec-contract` build itself even if this ChatGPT instance has already seen a prior contract hash, manifest, mapping/classification, receipt, handoff, or related historical contract state. For that substitute build, ChatGPT must re-read the current allowed `$spec-contract` inputs and derive the contract from those inputs rather than using the prior representation as an expected answer, template, or construction source. Historical comparison is permitted only after the substitute build reaches its own terminal result. This owner-level override supersedes narrower workflow language that would otherwise fail closed solely because the same ChatGPT context previously saw contract state; it does not authorize reuse of a stale handoff or weakening of deterministic contract validation, V2 hashing, exact-HEAD binding, coherence, coverage, or downstream semantic-certification requirements.
* **Substitute-build attestation:** a same-instance `$spec-contract` fallback must not falsely claim that no historical contract representation was ever visible to the ChatGPT context. Instead, retain the normal terminal `SPEC CONTRACT: VALID` result when every substantive contract gate passes and report `Build isolation: OWNER-AUTHORIZED IN-SESSION SUBSTITUTE`, `Historical contract state used as construction input: no`, and `Historical comparison performed before substitute terminal result: no`. For this authorized fallback, callers must accept that transparent substitute attestation in place of freshness-only fields such as `Build isolation: PASS` or `Prior contract representations supplied or inspected: 0`. The substitution changes only the separate-context/freshness assertion; it does not change any source-universe, mapping, hashing, handoff, coherence, exact-HEAD, proof, saturation, or finalization requirement.
