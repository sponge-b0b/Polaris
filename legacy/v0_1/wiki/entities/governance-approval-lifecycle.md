...
* A governed release may use a residual-risk acceptance only when its residual-risk scope exactly matches the requested scope, because a broader release needs a new explicit, attributable acceptance rather than a reinterpretation of an older record. (source: docs/adr/0010-governance-approval-lifecycle-contestability-residual-risk.md; docs/current/platform-architecture-ownership-ledger.md)
* Automated governance outcomes reached through canonical workflow execution and approval-required governed output-release evaluations must be recorded through `AutomatedDecisionAuditService` before the blocking outcome is surfaced, so the evidence-scoped review task is durable rather than a manual after-the-fact audit write. (source: docs/adr/0010-governance-approval-lifecycle-contestability-residual-risk.md; docs/adr/0001-runtime-workflow-platform-execution-boundaries.md; docs/current/platform-architecture-ownership-ledger.md)
* Review-task IDs, PostgreSQL uniqueness, and upsert conflict targets include intended sink alongside subject, evidence packet/version, review scope, and requested action, because approval for one controlled release boundary cannot authorize another. (source: docs/adr/0010-governance-approval-lifecycle-contestability-residual-risk.md; docs/current/platform-architecture-ownership-ledger.md)
