...
* Canonical orchestration creates a platform-owned execution correlation plus one transactionally bound Baseline evidence record and selection for each Baseline workflow execution, then request-scoped governed execution re-acquires and validates that record before audit capability issuance, because invocation authority and evidence selection cannot be inferred from transports, caller-selected identifiers, or transient runtime state. (source: docs/adr/0018-platform-governed-invocation-and-output-evidence-boundaries.md)
