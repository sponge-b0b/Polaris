...
def build_context() -> RuntimeContext:
    return RuntimeContext(
        ...
        workflow_id="pause_resume_workflow",
...
def build_two_wave_plan() -> WorkflowExecutionPlan:
    return WorkflowExecutionPlan(
        workflow_name="pause_resume_workflow",
        execution_id="execution-1",
        nodes={
...
@pytest.mark.asyncio
async def test_runtime_workflow_pauses_before_next_wave_and_resumes() -> None:
    ...
    await control_manager.request_pause(
        "execution-1",
        reason="external pause before next wave",
    ...
    assert snapshot.metadata["workflow_id"] == "pause_resume_workflow"
    assert snapshot.metadata["runtime_id"] == "runtime-1"

    event_types = [event.event_type for event in events]
    assert RuntimeEventType.WORKFLOW_PROGRESS_PAUSING in event_types
    assert RuntimeEventType.WORKFLOW_PROGRESS_PAUSED in event_types
    assert RuntimeEventType.WORKFLOW_PROGRESS_RESUMING in event_types
    assert RuntimeEventType.WORKFLOW_PROGRESS_RESUMED in event_types
    assert RuntimeEventType.WORKFLOW_PROGRESS_COMPLETED in event_types
