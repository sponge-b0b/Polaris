...
* **Governed invocation and output evidence composition** — accepted, implementation pending. Catalog-owned invocation facts must supply Baseline provenance to the request-scoped invocation path; each claim-bearing output materializer must own packet production and re-acquisition for its own release boundary. (source: docs/adr/0018-platform-governed-invocation-and-output-evidence-boundaries.md)
