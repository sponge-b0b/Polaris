...
* External presentation boundaries must consume the canonical application-level presentation decision and preserve its authority, provenance, disposition, and limitations without inferring stronger approval or authority locally, because renderers and transports must not become parallel risk, readiness, or governance policy owners. (source: docs/current/mcp-server-transport-boundary.md; docs/current/application-services-output-curation-workflow-output-curation.md)
* Governed CLI and backtest execution submit typed requests only; the request-scoped invocation service and output materializers resolve their respective authority and durable evidence, because transports must not become authority, evidence-selection, or provenance boundaries. (source: docs/adr/0015-dependency-composition-governed-execution-evidence-acquisition.md; docs/adr/0018-platform-governed-invocation-and-output-evidence-boundaries.md)
### Planned
