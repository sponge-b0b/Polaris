...
* Important canonical data requires a first-class typed field and schema migration rather than living only in generic metadata or JSON, because hidden semantics cannot safely support querying, governance, or projection rebuilds. (source: docs/adr/0004-persistence-curated-records-postgresql-system-of-record.md)
...
* **Durable actor and authorization attribution** — accepted, implementation pending: only actor-sensitive lifecycle records persist canonical principal references, protected durable mutations retain authorization-decision references where materially required, and derived records reuse canonical provenance rather than receiving universal `created_by` fields. (source: docs/adr/0023-identity-access-propagation-audit-attribution.md)
