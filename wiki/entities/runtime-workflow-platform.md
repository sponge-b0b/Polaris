...
* `WorkflowRegistry` binds each governed definition's deterministic `WorkflowIdentity` to its `RiskAuthorityContract`, and canonical orchestration creates an execution correlation then persists the tier-specific evidence selection before governed evaluation, because neither a transport nor request-scoped resolution may choose authority or replay evidence across executions. (source: docs/adr/0014-runtime-workflow-platform-governed-workflow-identity.md; docs/adr/0016-dependency-composition-governed-execution-evidence-resolution-lifecycle.md)
### Planned
