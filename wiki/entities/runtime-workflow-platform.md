...
* **Governed invocation and output evidence boundaries** — accepted, implementation pending. Canonical orchestration must validate Baseline provenance before a workflow invocation, while output materializers must produce and re-acquire provenance-bound Enhanced/Vigilant packets before their own governed output boundaries. (source: docs/adr/0018-platform-governed-invocation-and-output-evidence-boundaries.md)
