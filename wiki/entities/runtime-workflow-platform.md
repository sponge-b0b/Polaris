...
* Canonical orchestration creates one transactionally bound Baseline evidence record and selection for each Baseline workflow execution, then request-scoped governed execution re-acquires and validates that record before audit capability issuance, because invocation authority cannot be inferred from transports or transient runtime state. (source: docs/adr/0018-platform-governed-invocation-and-output-evidence-boundaries.md)
