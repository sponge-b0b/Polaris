...
* Strategy Advisory uses a dedicated typed result with `AVAILABLE`/`DEGRADED`/`UNAVAILABLE` status, a code-owned non-authoritative marker, canonical evidence-packet references, optional narrative, and typed advisory findings; it does not carry canonical strategy-selection fields or numeric advisory confidence, and model/runtime provenance remains outside the semantic payload. (source: docs/adr/0026-strategy-synthesis-advisory-output-contract.md)
...
* **Non-authoritative Strategy Advisory realization** — authority, workflow placement, and semantic output contract accepted; implementation pending while evidence validation, model routing/prompt ownership, persistence/publication, and readiness contracts are resolved. (source: docs/adr/0024-strategy-synthesis-non-authoritative-advisory-boundary.md; docs/adr/0025-strategy-advisory-workflow-placement.md; docs/adr/0026-strategy-synthesis-advisory-output-contract.md)
